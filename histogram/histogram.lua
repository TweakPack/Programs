...
local refreshRate      = 1 --screen update frequency(seconds)

local precisionDisplay = 100

local rsIoBlock = comp.redstone
local rsIoSide  = sides.left
...
elseif storageDevice == STORAGE_INDUCTION_MATRIX then
    storageObj = comp.induction_matrix
...
repeat
    ...
    elseif storageDevice == STORAGE_INDUCTION_MATRIX then
        input  = storageObj.getInput() * powerCalc
    ...
    graphHorizontal(2, 4, 78, 17, inputArray, true, false, 0x66CC00, 0xFFFFFF, 0xFFFFFF, 0x333333)
    ...
    if isCharging == true then
        ...
        drawLabel(2, 22, 158, 0x222222, 0xFFFFFF, storedLabel)
    end
